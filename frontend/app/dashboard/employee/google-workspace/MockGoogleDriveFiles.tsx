...
export function MockGoogleDriveFiles() {
  ...
  useEffect(() => {
    async function fetchFiles() {
      try {
        ...
        const isConnected = localStorage.getItem('googleWorkspaceConnected') === 'true';
        
        if (!isConnected) {
          // If not connected, don't try to fetch files
          setFiles([]);
          return;
        }
        ...
        // Fetch drive files using the adapter
        const driveFiles = await adapter.fetchResources<DriveFile>('driveFiles', {
